...
| RLS negative tests passed                     | Failed                       | TASK-004 User B'nin User A DB kayıtlarına erişimini reddetti; ancak reserved Storage INSERT policy helper false döndü ve authenticated Storage upload database HTTP 500 ile başarısız oldu. Forward fix + tam User A/B Storage rerun gerekir. |
| Authenticated CRUD tests passed               | In progress                  | TASK-004 User A araç create ve User B izolasyonunu remote'da doğruladı. Tam create/read/update/delete, persistence ve belge CRUD matrisi upload blocker'ı nedeniyle tamamlanmadı.                                                             |
| Private storage tests passed                  | Failed                       | Remote bucket private ve public/unsigned erişim reddi doğrulandı; 60 saniyelik URL expiry geçti. Buna karşın authenticated izinli upload `ATTACHMENT_UPLOAD_FAILED`/Storage database HTTP 500 verdi.                                          |
...
| Data deletion exists                          | In progress                  | Hesap silme backend E2E geçti; belge metadata + Storage object silme senaryosu izinli upload blocker'ı nedeniyle çalıştırılamadı. Cache, kısmi hata/retry ve Android kabulü ayrıca kanıtlanmalı.                                              |
...
| Document limits enforced                      | Failed                       | Remote schema 10 belge/25 MB ve bucket 5 MB ayarını içeriyor; fakat izinli upload çalışmadığı için 11. belge/25 MB E2E çalışmadı. 5 MB üstü istek kontrollü 413 yerine iki denemede HTTP 503 verdi.                                           |
| File validation enforced                      | Failed                       | Remote WebP ve sahte MIME doğru reddedildi; izinli PDF upload'u başarısız oldu ve 5 MB üstü kontrollü hata vermedi. PDF/JPEG/PNG pozitif matris, bozuk/aktif içerik ve cihaz fixture testi gerekir.                                           |
...
| No known blocker or critical bug              | Failed                       | TASK-004 authenticated upload/RLS database hatası ve 5 MB üstü istekte 503 buldu. Security advisor ayrıca callable `SECURITY DEFINER` helper ve kapalı leaked-password protection uyarısı verdi.                                              |
