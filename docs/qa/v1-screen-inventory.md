...
| -------------------------------- | -----: | -------------------------------------------------------------- |
| Gezilebilir route/ekran          |     27 | `src/app/**`, iki `_layout.tsx` hariç                          |
...
| Route içi `AppButton` occurrence |     38 | Kaynak taraması                                                |
| Route içi doğrudan `Pressable`   |     11 | Dinamik ortak kart/row bileşenleri hariç                       |
| `AppInput` occurrence            |     26 | Kaynak taraması                                                |
| `PasswordInput` occurrence       |      5 | Login 1 + kayıt 2 + yeni parola 2                              |
| `SelectField` occurrence         |      8 | Araç 2, kayıt 2, hatırlatıcı 2, gövde 1, belge 1               |
| `DateField` occurrence           |      5 | Kayıt 1, hatırlatıcı 1, ekspertiz 1, belge 2                   |
| Boş durum occurrence             |     13 | Altı `EmptyState` ve yedi araçsız güvenli durum                |
| Explicit tam ekran loading yolu  |     12 | Entry, recovery, tab ve entity edit route'ları                 |
| `ErrorBanner` occurrence         |     26 | Validation, repository, missing entity ve bağlantı durumları   |
...
| R09 | `/(tabs)/vehicle`                  | Araç özeti ve araç dosyası                     | Araç düzenle, Gövde/Ekspertiz/Not/Belgeler satırları                                                         | Loading; aktif araç yoksa araç oluşturma güvenli durumu                      | `src/app/(tabs)/vehicle.tsx`                  |
...
| R11 | `/(tabs)/settings`                 | Tema, hesap, bildirim, veri ve hukuk           | 3 tema radio'su; reset/logout/account delete; araç; bildirim; 4 section delete; araç delete; tek hukuk listesi | Confirmation/success alert'leri, toplu işlem disabled/loading, izin status metni | `src/app/(tabs)/settings.tsx`              |
...
| R14 | `/reminder/edit`                   | Hatırlatıcı create/edit/delete                 | Tür, başlık, tarih, km, tek-seçimli 7/3/1/0/özel gün uyarısı, kaydet/sil                                     | Loading/missing/error, en az bir hedef validation, success/confirm alert     | `src/app/reminder/edit.tsx`                   |
...
| R22 | `/legal/kvkk-notice`               | KVKK Aydınlatma Metni                          | Scroll ve Stack geri                                                                                         | Kullanıcıya dönük sade kartlı metin; canonical içerik korunur                | `src/app/legal/kvkk-notice.tsx`               |
| R23 | `/legal/privacy-policy`            | Gizlilik Politikası                            | Scroll ve Stack geri                                                                                         | Kullanıcıya dönük sade kartlı metin; canonical içerik korunur                | `src/app/legal/privacy-policy.tsx`            |
| R24 | `/legal/retention-and-deletion`    | Saklama ve Silme Politikası                    | Scroll ve Stack geri                                                                                         | Kullanıcıya dönük sade kartlı metin; canonical içerik korunur                | `src/app/legal/retention-and-deletion.tsx`    |
| R25 | `/legal/account-and-data-deletion` | Hesap ve Veri Silme açıklaması                 | Scroll ve Stack geri                                                                                         | Sade metin; gerçek silme eylemi Settings'tedir                              | `src/app/legal/account-and-data-deletion.tsx` |
| R26 | `/legal/kvkk-application`          | KVKK başvuru bilgileri                         | Scroll ve Stack geri                                                                                         | Sade metin; e-posta/URL metinleri otomatik link değildir                    | `src/app/legal/kvkk-application.tsx`          |
| R27 | `/legal`                           | Yasal ve gizlilik belge listesi                | Beş uygulama içi hukuk route'una ayrı satırlar                                                               | Statik liste; dış web alanına bağımlı değildir                              | `src/app/legal/index.tsx`, `legalRoutes.ts`   |
