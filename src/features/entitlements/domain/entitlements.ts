...
export const PLAN_ENTITLEMENTS: Readonly<Record<PlanId, Readonly<PlanEntitlements>>> = {
  free: {
    ...
    maxAttachmentBytesPerEntity: 15 * 1024 * 1024,
  ...
  premium: {
    ...
    maxAttachmentBytesPerEntity: 30 * 1024 * 1024,
...
export async function loadEntitlementsWithFallback(loadRecord: EntitlementRecordLoader) {
  try {
    return getEntitlements(await loadRecord());
  } catch {
...
export function canCreateVehicle(
  vehicleCount: number,
  entitlements: Pick<PlanEntitlements, 'maxVehicles'>,
) {
  return (
    Number.isInteger(vehicleCount) && vehicleCount >= 0 && vehicleCount < entitlements.maxVehicles
  );
}
