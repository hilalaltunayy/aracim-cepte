...
import { ActionSheet, AppHeader, Card, EmptyState, LoadingScreen, Screen, SectionHeader } from '@/shared/components/ui';
...
import { buildVehicleReport, REPORT_PERIOD_IDS, type ReportPeriodId } from '../domain/vehicleReports';
...
export function VehicleReportsScreen() {
  ...
  return <Screen>
    ...
    <Card style={styles.snapshot}><Text style={styles.eyebrow}>KAYITLI ARAÇ MALİYETİ</Text><Text style={styles.total}>{formatCurrency(total)}</Text><Text style={styles.snapshotCaption}>{report.period.label} içindeki yakıt, bakım ve diğer kayıtlar</Text><Comparison value={report.comparisons.total} /></Card>
...
const createStyles = ({ colors }: AppTheme) => StyleSheet.create({
  ...
  comparison: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: spacing.xs }, comparisonText: { color: colors.textSecondary, ...typography.caption }, comparisonMuted: { color: colors.textSecondary, ...typography.caption, marginTop: spacing.xs },
